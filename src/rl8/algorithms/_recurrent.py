...
@dataclass
class RecurrentAlgorithmConfig:
    ...
    #: Number of environment transitions to collect during :meth:`RecurrentAlgorithm.collect`.
    #: The environment resets according to ``horizons_per_env_reset``.
    #: Buffer size is [B, T] where T = horizon.
    horizon: int = 32

    #: Number of times :meth:`RecurrentAlgorithm.collect` can be called before resetting
    #: :attr:`RecurrentAlgorithm.env`. Increase this for cross-horizon learning. Default
    ...
    #: Whether to shuffle minibatches within :meth:`RecurrentAlgorithm.step`. Recommended, but
    ...
    #: Device :attr:`RecurrentAlgorithm.env`, :attr:`RecurrentAlgorithm.buffer`, and
    #: :attr:`RecurrentAlgorithm.policy` all reside on.
    device: Device | Literal["auto"] = "auto"

    def build(self, env_cls: EnvFactory) -> "RecurrentAlgorithm":
        """Build and validate a :class:RecurrentAlgorithm` from a config."""
        algo = RecurrentAlgorithm(env_cls, config=self)
        algo.validate()
        return algo
...
class RecurrentAlgorithm(
    ...
    Args:
        ...
        config: Recurrent algorithm config for building a recurrent PPO
            algorithm. See :class:`RecurrentAlgorithmConfig` for all parameters.
    ...
    def __init__(
        ...
        sgd_minibatch_size = (
            ...
            else num_envs * (horizon // config.seq_len)
        )
        self.hparams = RecurrentAlgorithmHparams(
            ...
            seqs_per_state_reset=config.seqs_per_state_reset,
